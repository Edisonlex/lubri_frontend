  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        <Button variant="ghost" size="sm" className="md:hidden h-8 w-8 sm:h-10 sm:w-10 p-0">
            ...
            <nav className="grid gap-1 px-2">
              {navigation.map((item) => (
                <Button
                  ...
                  className={cn(
                    "justify-start h-10 text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground transition-colors",
...
function DesktopSidebar({ className }: { className?: string }) {
  ...
  return (
    <motion.div
      ...
      className={cn("bg-sidebar border-r border-sidebar-border flex flex-col h-full", className)}
    >
      {/* Header */}
      <div className="p-2 sm:p-3 md:p-4 border-b border-sidebar-border">
        <div className="flex items-center justify-between">
          ...
          >
            {collapsed ? <ChevronRight className="h-3 w-3 sm:h-3.5 sm:w-3.5 md:h-4 md:w-4" /> : <ChevronLeft className="h-3 w-3 sm:h-3.5 sm:w-3.5 md:h-4 md:w-4" />}
          </Button>
        </div>
      </div>
